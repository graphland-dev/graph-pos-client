...
import { ActionIcon, Button, Drawer, Flex, Group, Input, Modal, NumberInput, Paper, Select, Space, Table, Text, Title } from "@mantine/core";
import { useDisclosure } from "@mantine/hooks";
import { showNotification } from "@mantine/notifications";
import { IconArrowsMaximize, IconBox, IconCalculator, IconCreditCard, IconDashboard, IconList, IconRefresh, IconUsers, IconX } from "@tabler/icons-react";
import { useEffect, useState } from "react";
import { useFieldArray, useForm } from "react-hook-form";
import { Link, useParams } from "react-router-dom";
import * as Yup from "yup";
import { calculateTaxAmount, getTotalProductsPrice, getTotalTaxAmount, getVatProfileSelectInputData } from "../purchases/create-purchase/utils/helpers";
...
const PosPage = () => {
	...
	function handleAddProductToList(productReference: ProductItemReferenceWithStockQuantity) {
		// console.log({ productReference: watch('products') });
		const productCart: ProductItemReference[] = watch("products");
		const index = productCart?.findIndex((item) => item.referenceId == productReference.referenceId);
		...
		if (index == -1) {
			...
			if (existingStock >= newStock) {
				...
				setValue(`products.${index}.quantity`, watch(`products.${index}.quantity`) + 1);
			}
		}
	}
	...
	const discountAmount = getDiscount(discountMode, discountValue, productsPrice);

	// sales vat calculate
	const salesVatAmount = getSalesVat(costAmount + productsPrice - discountAmount, taxRate);
	...
	useEffect(() => {
		if (selectedInvoice) {
			...
			setValue("discountMode", selectedInvoice?.discountMode ?? ProductDiscountMode.Percentage);
	...
	return (
		<div>
			{/* Header */}
			<Flex className="px-3 pb-0 h-[45px] border-b" justify={"space-between"} align={"center"}>
				<div className="font-bold">
					<Flex>
						<Button variant="subtle" color="accent" component={Link} to={"/"} leftIcon={<IconDashboard size={16} />}>
							Dashboard
						</Button>
						<Button variant="subtle" color="accent" leftIcon={<IconList size={16} />} component={Link} to={`/${params?.tenant}/inventory-management/invoices`}>
							Sales List
						</Button>
						<Button variant="subtle" color="accent" leftIcon={<IconUsers size={16} />} component={Link} to={`/${params?.tenant}/people/client`}>
							Customer List
						</Button>
						<Button
							variant="subtle"
				...
				<div className="flex items-center gap-3">
					<HoldList onSelectInvoice={setSelectedInvoice} holdList={holdList?.inventory__productInvoices?.nodes ?? []} onRefetchHoldList={refetchHoldList} />
					<IconArrowsMaximize
						onClick={() => {
					...
					<div className="lg:w-5/12">
						<Paper p={8} withBorder>
							<div className="grid grid-cols-2 gap-3 place-content-center">
								<ClientSearchAutocomplete
									prefilledClientId={watch("clientId") ?? selectedInvoice?.client?._id}
									onSelectClientId={(_id) => setValue("clientId", _id)}
								/>

								<ProductSearchAutocomplete formInstance={form} onSelectProduct={handleAddProductToList} />
							</div>
							<Space h={20} />
							...
							{Boolean(productFields?.length) && (
								<>
									<Table withBorder withColumnBorders>
										...
										<tbody>
											{productFields?.map((product: ProductItemReference, idx: number) => (
												<tr key={idx}>
													<td className="font-medium">{product?.name}</td>
													<td className="font-medium">
														<NumberInput
															w={100}
															onChange={(v) => setValue(`products.${idx}.quantity`, parseInt(v as string))}
															min={1}
													...
													<td className="font-medium">
														<NumberInput
															w={100}
															onChange={(v) => setValue(`products.${idx}.unitPrice`, parseInt(v as string))}
															min={1}
															value={watch(`products.${idx}.unitPrice`)}
														/>
													</td>
													<td className="font-medium text-center">
														{currencyNumberFormat(watch(`products.${idx}.quantity`) * watch(`products.${idx}.unitPrice`))}
													</td>
													<td className="font-medium">{product?.taxRate || 0}</td>
													<td className="font-medium">{currencyNumberFormat(calculateTaxAmount(watch(`products.${idx}`)))}</td>
													<td className="font-medium">
														{currencyNumberFormat(
															calculateTaxAmount(watch(`products.${idx}`)) + watch(`products.${idx}.quantity`) * watch(`products.${idx}.unitPrice`)
														)}
													</td>
													<td className="font-medium">
														<ActionIcon
											...
											<tr>
												...
												<td>{currencyNumberFormat(getTotalProductsPrice(watch("products")!))}</td>
												<td></td>
											</tr>
										</tbody>
									</Table>
						...
						<Paper p={15} withBorder>
							<div className="grid grid-cols-2 gap-3">
								<div>
									<Input.Wrapper size="md" error={<ErrorMessage name="discountType" errors={errors} />}>
										<Select
											label="Discount Mode"
									...
									<Input.Wrapper size="md" error={<ErrorMessage name="transportCost" errors={errors} />}>
										<NumberInput
											label="Transport Cost"
											size="md"
											onChange={(e) => setValue("costAmount", parseInt(e as string))}
								...
								<div>
									<Input.Wrapper size="md" error={<ErrorMessage name="discountAmount" errors={errors} />}>
										<NumberInput
											label="Discount"
											radius={0}
											size="md"
											...
											onChange={(e) => setValue("discountValue", parseInt(e as string))}
									...
									<Input.Wrapper size="md" error={<ErrorMessage name="invoiceTax" errors={errors} />}>
										<Select
											label="Invoice Tax"
											...
											data={getVatProfileSelectInputData(vatProfile?.setup__vats?.nodes as Vat[])}
											onChange={(e) => setValue("taxRate", parseInt(e as string)!)}
										/>
									</Input.Wrapper>
								</div>
							</div>
							...
							<Paper withBorder p={"sm"} mb={"xl"}>
								...
								<Flex justify={"space-between"}>
									...
									<Text>{currencyNumberFormat(productsPrice + costAmount) || 0} BDT</Text>
								</Flex>
								<Flex justify={"space-between"}>
									<Text fw={"bold"}>
										Discount
										{watch("discountMode") === "PERCENTAGE" && ` (${watch("discountValue")}%)`}
									</Text>
									<Text>{currencyNumberFormat(getDiscount(discountMode, discountValue, productsPrice)) || 0} BDT</Text>
								</Flex>
								<Space h={"sm"} />
								<div className="flex justify-between p-3 text-xl font-bold text-center text-black bg-indigo-200 rounded-sm">
									<div>Net Total (Subtotal - Discount)</div> <div>{currencyNumberFormat(getNetAmount())} BDT</div>
								</div>
							</Paper>
							...
							{/* hold modal */}
							<Modal opened={openedHoldModal} onClose={holdModalHandler.close} title="">
								<HoldAction
									formData={
										{
							...
							{/* payment form */}
							<Drawer opened={openedPaymentModal} onClose={paymentModalHandler.close} title="Multiple payment to invoice" size={"lg"} position="right">
								<PaymentForm
									formData={{
										clientId: watch("clientId"),
										products,
										...
										discountAmount,
										discountPercentage: discountMode === ProductDiscountMode.Percentage ? discountValue : 0,
										discountMode,
							...
							<Group position="apart">
								<Button size="md" type="submit" onClick={() => setAction("ADD_TO_HOLD_LIST")} disabled={!watch("products")?.length || !watch("clientId")}>
									Hold
								</Button>
								<Button
									size="md"
...
const Pos_Form_Validation_Schema = Yup.object().shape({
	...
	products: Yup.array().required().min(1, "You must have to select at least one product").label("Purchase products"),
