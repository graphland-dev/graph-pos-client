...
import { Pos_Brands_Query, Pos_Categories_Query, Pos_Products_Query } from "../utils/query.pos";
		...
		<div className="flex flex-col h-[calc(100vh-46px)]">
			...
			<div className="grid flex-none h-[40px] place-content-center grid-cols-2 gap-2">
				<Select
					...
					data={getSelectInputData(categories?.inventory__productCategories?.nodes)}
			...
			<div className="flex-1 overflow-y-auto">
				<div className={`grid grid-columns--${itemsGridColumnCount} gap-2`}>
					{productsData?.inventory__products.nodes?.map((product, index) => (
						<PosItemCard key={index} product={product} onClick={(_product) => handleEmitProduct(_product)} />
					))}
				</div>
			</div>

			<EmptyState visible={!productsData?.inventory__products.nodes?.length && !isProductsFetching} label={"No products found with your filter!"} />
			...
			<div className="flex-none h-[40px] flex justify-between items-center px-2">
				...
				<div className="flex justify-center gap-2">
					...
					<Button
						onClick={() => {
							if (page === productsData?.inventory__products.meta?.totalPages) return;
							setPage(page + 1);
						}}
					>
						Next
